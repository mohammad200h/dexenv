...
import dexenv
from dexenv.envs.dclaw_base import LeapXelaBase
...
class LeapXelaMultiObjs(LeapXelaBase):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        leapXela_asset, leapXela_dof_props = self.get_dclaw_asset()
        ...
        if self.obs_type == "full_state":
            ...
            for ft_handle in self.fingertip_handles:
                self.gym.create_asset_force_sensor(leapXela_asset, ft_handle, sensor_pose)

        leapXela_start_pose = self.get_dclaw_start_pose()
        object_start_pose = self.get_object_start_pose(leapXela_start_pose)
        ...
        leapXela_rb_count = self.gym.get_asset_rigid_body_count(leapXela_asset)
        object_rb_count = self.gym.get_asset_rigid_body_count(object_assets[0])

        self.object_rb_handles = list(range(leapXela_rb_count, leapXela_rb_count + object_rb_count))
        self.object_handles = []
        ...
        for i in range(self.num_envs):
            ...
            self.create_hand_actor(env_ptr=env_ptr,
                                   dclaw_asset=leapXela_asset,
                                   dclaw_start_pose=leapXela_start_pose,
                                   dclaw_dof_props=leapXela_dof_props,
                                   env_id=i)
            # add object
